...
export default function SendEmail() {
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor }) => {
      // Optional: Handle content updates if needed
  ...
  useEffect(() => {
    async function loadApplicants() {
      ...
      try {
        ...
        setSelectedApplicants([]); // Reset selection when job changes
      } catch (err) {
  ...
  const handleSend = async () => {
    try {
      ...
      if (sendResult.failedRecipients?.length) {
        setSuccessMessage(`Email sent successfully but failed for: ${sendResult.failedRecipients.join(', ')}`);
      } else {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex justify-center border-b border-gray-200">
        <div className="text-center px-8 py-2">
          <span className="text-primary font-medium">New Message</span>
          <span className="mx-3 text-gray-300">|</span>
          <span className="text-gray-400 cursor-not-allowed">Continue Thread (Coming Soon)</span>
        </div>
      </div>
